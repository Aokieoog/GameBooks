<template>
  <el-popover placement="left" z-index="99999" :width="700" trigger="click">
    <template #reference>
      <el-button link type="primary" size="small" @click="getSoldOrders"
        >出售</el-button
      >
    </template>
    ...
    <div style="padding: 5px 0">
      <el-progress
        :stroke-width="3"
        :percentage="(total_quantity / props.call).toFixed(2) * 100"
      >
      </el-progress>
    </div>
    ...
    <el-table :data="tosellData" border max-height="400">
      ...
      >
        <template #default="scope">
          <span style="color: #f75e02">{{
            util.numPad(scope.row.totalRevenue)
          }}</span>
        </template></el-table-column
      >
      <el-table-column fixed="right" label="状态">
        <template #default="scope">
          <el-button
            ...
            type="primary"
            size="small"
            @click.prevent="deleteSoldOrders(scope.row.id, scope.row.quantity)"
          >
            删除
          </el-button>
        </template>
...
const getSoldOrders = async () => {
  ...
  Jx3Store.updateOrderTotalSellingPrice(
    props.sellPriceprops,
    totalSellingPrice,
  ).then(() => {
    Jx3Store.orderInquiry();
  });
