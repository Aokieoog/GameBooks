  ...
  <div>
    <div class="navbar">
      ...
      <div>
        ...
        <span :style="sumtasc > 0 ? 'color: red;' : 'color:green'">{{
          sumtasc
        }}</span>
      </div>
      <div>
        <span class="spannav" style="margin: 1.25rem">已获利润:</span>
        <span :style="sumtakc > 0 ? 'color: red;' : 'color:green'">{{
          sumtakc
        }}</span>
      </div>
      <div>
        <el-button :disabled="!loginshow" color="#8f58fd" @click="addkcheader"
          >增加库存</el-button
        >
      </div>
      <!-- 登录 -->
      <div style="display: flex; align-items: center">
        ...
        <el-dialog
          ...
          :title="zhuceshow ? '注册账号' : passshow ? '登录账号' : '找回密码'"
          width="500"
          center
        >
          ...
          >
            ...
            <el-button
              ...
              style="width: 12.5rem"
              type="primary"
              @click="zhuceshow ? Zhuceq() : FindPassword()"
              >确认</el-button
            >
          </div>
        </el-dialog>
      ...
      >
        ...
        >
          ...
          <el-form-item label="数量" prop="quantity">
            ...
            <el-input
              style="width: 3.125rem; margin-left: 1.25rem"
              maxlength="3"
          ...
          <el-form-item label="成本" prop="cost">
            ...
            <el-input
              style="width: 3.4375rem; margin-left: 1.25rem"
              maxlength="4"
        ...
        <div
          style="display: flex; justify-content: center; margin-top: 1.25rem"
        >
          <el-button style="width: 12.5rem" type="primary" @click="addkc"
            >确认</el-button
          >
        </div>
      </el-dialog>
    ...
    <div v-show="optionvalue == 0" class="tablebox">
      ...
      >
        ...
        <el-table-column prop="wblprice1" label="单件利润" sortable>
          <template #default="scope">
            <span
              :style="{
                color:
                  scope.row.wblprice - scope.row.cost > 0 ? 'red' : 'green',
              }"
              >{{ scope.row.wblprice - scope.row.cost }}</span
            >
          </template>
        </el-table-column>
        <el-table-column prop="wblprice2" label="总利润" sortable>
          <template #default="scope">
            <span
              ...
              >{{
                scope.row.wblprice * scope.row.quantity -
        ...
        <el-table-column prop="desc1" label="操作" width="190">
          <template #default="scope">
            ...
            <el-button link type="primary" @click="Bianji(scope.row)"
              >编辑</el-button
            >
            <el-button link type="danger" @click="Shanchu(scope.row)"
              >删除</el-button
            >
          </template>
        </el-table-column>
      ...
      <div class="demo-image__lazy" v-show="imageshow">
        ...
        <el-button style="margin: 10px 10px" @click="imageshow = false"
          >关闭</el-button
        >
      </div>
    </div>
    ...
    <div v-show="optionvalue == 1" class="tablebox">
      ...
      >
        ...
        <el-table-column prop="saleprice" label="出售单价" sortable>
          <template #default="scope">
            <span style="color: #f75e23">{{ scope.row.saleprice }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="wblprice1" label="总利润" sortable>
          <template #default="scope">
            <span
              :style="{
                color:
                  scope.row.saleprice - scope.row.cost > 0 ? 'red' : 'green',
              }"
              >{{
    ...
    <!-- 操作栏弹窗 -->
    <el-dialog
      v-model="dialogVisibleb"
      :title="csshow ? '出售' : bjshow ? '编辑' : '删除'"
      width="500"
      center
      :before-close="handleClose"
    >
...
async function FindPassword() {
  ...
  try {
    ...
    if (res.status === 200) {
      ElMessage.success('密码重置链接发送到电子邮件');
    }
  } catch (error) {
...
function Bianji(params) {
  ...
  bjshow.value = true;
  const { zone, quantity, cost, open_at, name, desc, id, sale, saleprice } =
    params;
  radio1.value = zone;
...
const createFilter = (queryString) => {
  return (restaurant) => {
    return (
      restaurant.subalias.toLowerCase().indexOf(queryString.toLowerCase()) !==
      -1
    );
  };
};
...
async function addkc() {
  const { open_at, name, cost, desc, zone, quantity, id, sale, saleprice } =
    ruleForm;
  if (bjshow.value) {
    let params = {
      open_at,
